...
func New(id string, name string, c *smarthome_sdk.Connection) {
	...
	if err := c.CreateHomescript(smarthome_sdk.HomescriptRequest{
		Id:   id,
		Name: name,
	}); err != nil {
		switch err {
		case smarthome_sdk.ErrUnprocessableEntity:
...
func Delete(id string, purgeOrigin bool, c *smarthome_sdk.Connection) {
	if err := removeProjectFiles(id); err != nil {
